...
void _dynarray_resize(struct dynarray* da, int new_capacity) {
  ...
  for (int i = 0; i < da->size; i++) {
    indx = (da->start + i) % da->capacity;
    new_data[i] = da->data[indx];
  }
...
void dynarray_insert(struct dynarray* da, void* val) {
   ...
   */
  phys = (da->start + da->size) % da->capacity;
  da->data[phys] = val;
  da->size++;
}
...
void* dynarray_rm_front(struct dynarray* da) {
  void* front = da->data[da->start];
