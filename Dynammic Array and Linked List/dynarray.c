...
struct dynarray* dynarray_create() {
  ...
  new_arr->capacity = 2;
  new_arr->data = malloc(2* sizeof(void*));
  return new_arr;
}
...
/* This function doubles the array that it is passed. It does this by 
 * creating a enw pointer to a dynamic area, then doubling its capacity
 * and copying over the data from the old array. 
 *
 *Params:
 *  da - the dynamic array that needs to be doubled. May not be NULL.
 */
struct dynarray* double_dynarray(struct dynarray* da) {
  int i;
  struct dynarray* temp_arr = malloc(sizeof(struct dynarray));
  temp_arr->capacity = da->capacity;
  temp_arr->size = da->size;
  temp_arr->data = malloc(da->capacity * sizeof(void*));
  for(i = 0; i < temp_arr->size; i++) {
    temp_arr->data[i] = da->data[i];
  }
  free(da->data);
  free(da);  
    
  struct dynarray* doubled_arr = malloc(sizeof(struct dynarray));
  doubled_arr->capacity = temp_arr->capacity * 2;
  doubled_arr->size = temp_arr->size;
  doubled_arr->data = malloc(doubled_arr->capacity * sizeof(void*));
  for(i = 0; i < doubled_arr->size; i++) {
    doubled_arr->data[i] = temp_arr->data[i];
  }
  free(temp_arr->data);
  free(temp_arr);
  return doubled_arr;
...
void dynarray_insert(struct dynarray* da, void* val) {
  ...
  if((da->size + 1) > da->capacity) {
    //If it is too big make a temperary array to store all the current info in
    da = double_dynarray(da);
  }
  //printf("size == %d, capacity == %d ", da->size, da->capacity);
  da->data[da->size] = val;
  da->size = da->size + 1;
...
void dynarray_remove(struct dynarray* da, int idx) {
  ...
  for(i = 0; i < (da->size - (idx+1)); i++) {
    da->data[idx+i] = da->data[idx+i+1];
  }
