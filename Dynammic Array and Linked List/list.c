...
struct list* list_create() {
  struct list* new_arr = malloc(sizeof(struct list));
  return new_arr;
}
...
void list_remove(struct list* list, void* val, int (*cmp)(void* a, void* b)) {
  ...
  new_link = list->head;
  while(found == false) {
    if(cmp(val, new_link->val) == 0) {
      if(new_link == list->head) {
      	list->head = new_link->next;
      } else {
        prev->next = new_link->next;
      }
      free(new_link);
      break;
    }
